...
// Helper function
const getWindowsQuery = () =>
  JSON.parse(execSync('yabai -m query --windows')).filter((e) => e['has-ax-reference']);
const getDisplaysQuery = () => JSON.parse(execSync('yabai -m query --displays'));
...
const validateSetup = (presetConfig) => {
  const displaysQuery = getDisplaysQuery();
  ...
  const displayErrorMessageList = presetConfig.map(
    ({ display_index: displayIndex, spaces }) => {
      if (
        ...
        const maxSpaceNumber = getMaximumSpaceIndex(displaysQuery, displayIndex);
...
const populateDisplay = (presetConfig) => {
  const windowsQuery = getWindowsQuery();
  const displaysQuery = getDisplaysQuery();
  ...
  if (dumpDisplayIndex != null && dumpSpaceIndex != null) {
    const minDumpSpaceIndex = getMinimumSpaceIndex(displaysQuery, dumpDisplayIndex);
